...
(defn partially-applied-attr
  ...
  (let [{:keys [attributes all-attributes]} token-type-props
    ...
    (when is-applied
      (str/join ", " (mapv attribute-dictionary filtered-keys)))))
...
(defn token-pill-tooltip
  ...
  [theme-token viewer shape token-type-props token half-applied]
  (let [{:keys [name value resolved-value errors type]} token
    ...
    (cond
      ...
      ;; If the token is applied and the user is a viewer, show the details
      (and is-applied? viewer)
      (->> [base-title
...
(mf/defc token-pill
  ...
  (let [{:keys [name value errors]} token
        ...
        on-hover
        (mf/use-fn
         (mf/deps selected-shapes viewer)
         (fn [event]
           (let [node (dom/get-current-target event)
                 calculate-tooltip (token-pill-tooltip theme-token viewer (first selected-shapes) token-type-props token half-applied)]
             (dom/set-attribute! node "title" calculate-tooltip))))]
    ...
    [:button {:class (stl/css-case :token-pill true
                                   ...
                                   :token-pill-viewer viewer
                                   :token-pill-applied-viewer (and viewer
                                                                   (or half-applied full-applied))
                                   :token-pill-invalid-viewer (and viewer
                                                                   errors?)
                                   :token-pill-invalid-applied-viewer (and viewer
